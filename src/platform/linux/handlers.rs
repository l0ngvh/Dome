...
use crate::core::{
    Dimension, Length, LimitObservation, LimitUpdate, MonitorLayout, VisiblePlacements, WindowId,
    WindowRestrictions,
};
...
fn window_frame(placements: &VisiblePlacements, id: WindowId) -> Option<Dimension> {
    for mp in &placements.monitors {
        ...
        for wp in tiling_windows {
            if wp.id == id {
                return Some(wp.visible_border_box.to_dimension());
            }
        }
...
impl CompositorHandler for DomeState {
    ...
    fn commit(&mut self, surface: &WlSurface) {
        ...
        if let Some(window_id) = with_states(surface, |states| {
            ...
            // Wayland reports content-box constraints; core adds the border when it
            // converts to border-box. A protocol value of 0 means no limit.
            if min.w > 0 || min.h > 0 || max.w > 0 || max.h > 0 {
                let limit = |v: i32| {
                    if v > 0 {
                        LimitUpdate::Set(Length::new(v as f32))
                    } else {
                ...
                self.hub.set_window_constraint(
                    window_id,
                    LimitObservation {
                        min_width: limit(min.w),
                        min_height: limit(min.h),
                        max_width: limit(max.w),
                        max_height: limit(max.h),
                    },
                );
...
impl XdgShellHandler for DomeState {
    ...
    fn new_toplevel(&mut self, surface: ToplevelSurface) {
        ...
        // Placeholder rect. The layout engine assigns real geometry.
        let placeholder = crate::core::PixelRect::ZERO;
        let Some(window_id) =
            self.hub
                .insert_window(metadata, placeholder, crate::core::WindowRestrictions::None)
        else {
    ...
    fn grab(&mut self, surface: PopupSurface, seat: WlSeat, serial: Serial) {
        ...
        if let Ok(root) = find_popup_root_surface(&popup_kind) {
            ...
            if let Ok(mut grab) = self.popups.grab_popup(root, popup_kind, &seat, serial) {
...
impl DataControlHandler for DomeState {
...
impl WlrLayerShellHandler for DomeState {
    ...
    fn new_popup(
        &mut self,
        _parent: wlr_layer::LayerSurface,
...
delegate_data_control!(DomeState);
