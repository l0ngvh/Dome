...
pub(super) fn init_winit_backend(
    ...
    // winit selects its backend from the host WAYLAND_DISPLAY / DISPLAY. Dome no
    // longer overwrites its own process WAYLAND_DISPLAY, so winit connects to the
    // host compositor, or falls back to X11, rather than to Dome itself.
    let (mut backend, mut winit_evt) =
        winit::init::<GlowRenderer>().map_err(|e| anyhow!("winit init failed: {e}"))?;

    // Build our own glow 0.17 context sharing the winit EGL context.
    // smithay 0.7 pins glow 0.16 internally so backend.renderer().with_context()
    // returns the wrong Context type for egui_glow 0.34.
    let gl = {
