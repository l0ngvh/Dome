...
fn connector_connected(
    ...
    let dimension = crate::core::Dimension::new(
        crate::core::Length::new(0.0),
        crate::core::Length::new(0.0),
        crate::core::Length::new(wl_mode.size.w as f32),
        crate::core::Length::new(wl_mode.size.h as f32),
    );

    // First connector: add real monitor, remove dummy "primary"
    let monitor_id = if state.monitor_outputs.is_empty() {
        let dummy = state.hub.focused_monitor();
        // TODO: plumb real fractional scale from wl_output
        let id = state.hub.add_monitor(output_name.clone(), dimension, 1.0f32);
        state.hub.remove_monitor(dummy, id);
        id
    } else {
        state.hub.add_monitor(output_name.clone(), dimension, 1.0f32)
    };
...
fn reposition_outputs(state: &mut DomeState) {
    ...
    outputs.sort_by_key(|o| {
        state.space.output_geometry(o).map(|g| g.loc.x).unwrap_or(0)
    });
    let mut x = 0i32;
    ...
    if let Some(device) = udev.backends.get_mut(&node) {
        if let Some(surface) = device.surfaces.remove(&crtc) {
            ...
            if !state.monitor_outputs.is_empty() {
                let fallback = *state.monitor_outputs.keys().next().unwrap();
                state.hub.remove_monitor(surface.monitor_id, fallback);
            }
...
fn render_egui_to_buffer(
    ...
    let (meshes, textures_delta, pixels_per_point) = state.build_egui_shapes(width, height, Some(monitor_id));
