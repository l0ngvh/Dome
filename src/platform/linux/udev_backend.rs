...
pub(super) struct UdevData {
    ...
    pub(super) gpus: GpuManager<GbmGlesBackend<GlesRenderer, DrmDeviceFd>>,
...
fn device_added(state: &mut DomeState, node: DrmNode, path: &Path) -> Result<()> {
    ...
    if need_egui_init {
        ...
        // Init dmabuf on first GPU
        let udev = state.udev_data.as_mut().unwrap();
        let renderer = udev.gpus.single_renderer(&render_node)?;
...
fn select_mode_index<T>(modes: &[T], is_preferred: impl Fn(&T) -> bool) -> Option<usize> {
    modes
        .iter()
        .position(|m| is_preferred(m))
...
fn connector_connected(
    ...
    let Some(mode_id) = select_mode_index(connector.modes(), |m| {
    ...
    // Position side-by-side: x = rightmost edge of existing outputs
    ...
    let reported = crate::core::ReportedMonitor {
        ...
        // TODO: plumb real fractional scale from wl_output
        work_area: crate::core::PixelRect::new(0, 0, wl_mode.size.w, wl_mode.size.h),
        scale: 1.0,
        cg_display_id: None,
        gdi_device: None,
    };

    // The first real output replaces the placeholder primary from Hub::new. The
    // monitor model forbids removing the primary, so update it in place instead
    // of the old add-then-remove-dummy dance.
    let monitor_id = if state.monitor_outputs.is_empty() {
        let primary = state.hub.focused_monitor();
        state.hub.update_monitor(primary, reported, None);
        primary
    } else {
        state.hub.add_monitor(reported)
    };
    state.monitor_outputs.insert(monitor_id, output.clone());

    let drm_output = match device
        .drm_output_manager
        .initialize_output::<_, DrmRenderElement<_>>(
            crtc,
            drm_mode,
            &[connector.handle()],
            &output,
...
fn reposition_outputs(state: &mut DomeState) {
    let mut outputs: Vec<Output> = state.space.outputs().cloned().collect();
    outputs.sort_by_key(|o| state.space.output_geometry(o).map(|g| g.loc.x).unwrap_or(0));
    let mut x = 0i32;
...
fn connector_disconnected(state: &mut DomeState, node: DrmNode, crtc: crtc::Handle) {
    ...
    state.monitor_outputs.remove(&surface.monitor_id);

    if !state.monitor_outputs.is_empty() {
        let primary = state.hub.primary_monitor();
        if surface.monitor_id == primary {
            // Core forbids removing the primary monitor, so re-home it onto a
            // surviving output and displace that output's monitor. Mirrors the
            // macOS/Windows primary-replacement path.
            let (survivor_id, survivor_output) = state
                .monitor_outputs
                .iter()
                .map(|(id, output)| (*id, output.clone()))
                .next()
                .expect("a survivor exists while monitor_outputs is non-empty");
            state.hub.update_monitor(
                primary,
                reported_from_output(&survivor_output),
                Some(survivor_id),
            );
            state.monitor_outputs.remove(&survivor_id);
            state.monitor_outputs.insert(primary, survivor_output);
            remap_surface_monitor(state, survivor_id, primary);
        } else {
            state.hub.remove_monitor(surface.monitor_id);
        }
    }

    reposition_outputs(state);
    state.sync_window_positions();
}

fn reported_from_output(output: &Output) -> crate::core::ReportedMonitor {
    let size = output.current_mode().map(|m| m.size).unwrap_or_default();
    crate::core::ReportedMonitor {
...
/// Re-point every DRM surface tagged with `from` to `to` after a monitor
/// re-home, so rendering keeps filtering by a live monitor id.
fn remap_surface_monitor(state: &mut DomeState, from: MonitorId, to: MonitorId) {
    let udev = state.udev_data.as_mut().unwrap();
    for device in udev.backends.values_mut() {
        for surface in device.surfaces.values_mut() {
            if surface.monitor_id == from {
                surface.monitor_id = to;
            }
        }
    }
}
...
fn ensure_egui_fbo(gl: &glow::Context, fbo: &mut Option<EguiFbo>, width: u32, height: u32) {
    use glow::HasContext;
...
fn render_egui_to_buffer(
    ...
    let (meshes, textures_delta, pixels_per_point) =
        state.build_egui_shapes(width, height, Some(monitor_id));
    ...
    painter.paint_and_update_textures([width, height], pixels_per_point, &meshes, &textures_delta);
...
fn render_surface(state: &mut DomeState, node: DrmNode, crtc: crtc::Handle) {
    ...
    let egui_buf = {
        ...
        let monitor_id = surface.monitor_id;
        let gl = udev.gl.clone();
        let mut fbo = udev.egui_fbo.take();

        let buf = match (gl, output_mode) {
            (Some(gl), Some(mode)) => {
                if let Ok(mut r) = udev.gpus.single_renderer(&udev.primary_gpu) {
                    // SAFETY: single_renderer produced a valid GL renderer for primary_gpu.
                    unsafe {
                        if let Err(e) = r.as_mut().egl_context().make_current() {
                            tracing::warn!("failed to make EGL context current: {e:#}");
                        }
                    }
                }
    ...
    // Cursor — only on the output containing the pointer
    let pointer_loc = state.seat.get_pointer().unwrap().current_location();
    let output_geo = state.space.output_geometry(&output);
    let cursor_on_this_output = output_geo
        .map(|geo| geo.to_f64().contains(pointer_loc))
        .unwrap_or(false);
        ...
        match &state.cursor_status {
            ...
            CursorImageStatus::Named(icon) => {
                if !udev.cursor_cache.contains_key(icon) {
                    if let Some(entry) =
                        load_xcursor(&udev.cursor_theme, icon.name(), udev.cursor_size)
                    {
                        udev.cursor_cache.insert(*icon, entry);
                    }
                }
                let (buf, hotspot) = udev
                    .cursor_cache
                    .get(icon)
                    .map(|(b, h)| (b, *h))
...
#[cfg(test)]
mod tests {
    use super::select_mode_index;

    #[test]
    fn no_modes_returns_none() {
        assert_eq!(select_mode_index::<bool>(&[], |&p| p), None);
    }

    #[test]
    fn no_preferred_selects_first() {
        assert_eq!(select_mode_index(&[false, false], |&p| p), Some(0));
    }

    #[test]
    fn selects_the_preferred_mode() {
        assert_eq!(select_mode_index(&[false, true, false], |&p| p), Some(1));
    }

    #[test]
    fn selects_the_first_preferred_when_several() {
        assert_eq!(select_mode_index(&[true, true], |&p| p), Some(0));
    }
}
