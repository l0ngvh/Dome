...
use crate::core::{Dimension, Length, Logical, MonitorLayout};
use crate::overlay;
...
impl DomeState {
    pub(super) fn render_winit(&mut self) {
        ...
        match &self.cursor_status {
            CursorImageStatus::Named(icon) => {
                ...
                winit_data.backend.window().set_cursor(
                    smithay::reexports::winit::window::Cursor::Icon((*icon).into()),
                );
            }
...

        {
            let (renderer, mut framebuffer) =
                winit_data.backend.bind().expect("failed to bind backend");
        ...
        self.space.elements().for_each(|window| {
            window.send_frame(&winit_data.output, elapsed, Some(Duration::ZERO), |_, _| {
                Some(winit_data.output.clone())
            });
        });

        {
            ...
            for layer in layer_map.layers() {
                layer.send_frame(&winit_data.output, elapsed, Some(Duration::ZERO), |_, _| {
                    Some(winit_data.output.clone())
                });
            }
        }
    ...
    fn render_egui_overlays(&mut self, width: u32, height: u32) {
        ...
        let (meshes, textures_delta, pixels_per_point) =
            self.build_egui_shapes(width, height, None);
    ...
    pub(super) fn build_egui_shapes(
        ...
        let theme = Theme::from_flavor(self.config.theme);
        let border =
            overlay::BorderMetrics::from_thickness(Length::from_pixels(self.config.border_size));
        ...
        let output = self.egui_ctx.run_ui(raw_input, |ui| {
            for mp in &placements.monitors {
                ...
                for wp in tiling_windows {
                    let frame = wp.border_box.to_dimension();
                    let vf = wp.visible_border_box.to_dimension();
                ...
                for cp in containers {
                    let vf = cp.visible_border_box.to_dimension();
                    let clip = egui::Rect::from_min_size(
                        egui::pos2(vf.x.value(), vf.y.value()),
                        egui::vec2(vf.width.value(), vf.height.value()),
                    );
                    ...
                    let logical = overlay::LogicalTiledContainer {
                        id: cp.id,
                        frame: cp.border_box.to_dimension(),
                        visible_frame: vf,
                        tab_bar_height: Length::from_pixels(cp.tab_bar_band.height()),
                    ...
                    overlay::show_container(&mut child, &logical, &theme, border, origin);
                }
            }
        });
