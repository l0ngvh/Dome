...
use crate::config::watch::{load_or_default, start_config_watcher, start_file_watcher};
use crate::config::{Appearance, Config, PreferredLayouts, bootstrap::resolve_config_path, paths};
...
pub fn run_app(config_path: Option<String>, layout_path: Option<String>) -> Result<()> {
    ...
    let layout = load_or_default(&layout_path, PreferredLayouts::load);
    ...
    let (keymap_tx, keymap_rx) = mpsc::channel::<KeymapState>();
    ...
    let dome_thread = thread::spawn({
        ...
        move || {
            let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                ...
                keymap.update_keymaps(config.keymaps.clone());
                let tid = unsafe { GetCurrentThreadId() };
                init_tx.send((tid, config.clone())).ok();
                run_dome(config, layout, main_thread_id, keymap, runtime, logger);
            }));
            if result.is_err() {
...
fn run_dome(
    ...
    let wt_appearance = config.appearance.clone();
    let window_thread = thread::spawn(move || {
        let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            run_window_thread(domain_thread_id, handshake_tx, wt_appearance);
        }));
        if result.is_err() {
