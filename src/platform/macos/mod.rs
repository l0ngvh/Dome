...
use crate::config::{Config, PreferredLayouts, bootstrap::resolve_config_path, paths};
use crate::ipc;
...
pub fn run_app(config_path: Option<String>, layout_path: Option<String>) -> anyhow::Result<()> {
    ...
    let hub_thread = thread::spawn({
        ...
        move || {
            std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                ...
                event_loop::run_dome(dome, event_rx, logger, bundle_path, env);
            }))
            .ok();
        }
    });
    ...
    let (ui, sender) = Ui::new(mtm, event_tx, event_listener, config.appearance);
