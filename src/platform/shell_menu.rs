...
/// `TRAY_CMD_WORKSPACE_BASE`, so `id_to_action` can recover it.
///
/// `gray_detached` grays a detached monitor's submenu. macOS passes true. Windows passes
/// false, because a grayed Win32 popup will not expand, which would strand the parked rows
/// out of reach.
pub(crate) fn build_menu(workspaces: &[WorkspaceInfo], gray_detached: bool) -> Vec<MenuEntry> {
    ...
    for group in group_by_monitor(workspaces) {
        ...
        let enabled = !(group.detached && gray_detached);
...
pub(crate) fn truncate_tooltip(name: &str) -> String {
...
pub(crate) fn focused_tooltip(workspaces: &[WorkspaceInfo]) -> String {
    workspaces
        .iter()
        .find(|w| w.is_focused)
...
pub(crate) fn id_to_action(id: u32, workspaces: &[WorkspaceInfo]) -> Option<Action> {
    if id == 0 {
        return None;
    }
    if id == TRAY_CMD_EXIT {
...
mod tests {
    ...
    #[test]
    fn id_attached_focus_carries_monitor() {
        let list = vec![
    ...
    #[test]
    fn id_parked_emits_unified_monitor() {
        let list = vec![
    ...
    #[test]
    fn id_workspace_out_of_range() {
        assert!(id_to_action(TRAY_CMD_WORKSPACE_BASE + 5, &[]).is_none());
    }
    ...
    fn build_menu_groups_and_appends_exit() {
        ...
        let entries = build_menu(&list, false);
    ...
    #[test]
    fn build_menu_grays_detached_when_requested() {
        let list = vec![
            ws("Main", "Mon1", WorkspaceState::Attached, true, true),
            ws("Ghost", "Gone", WorkspaceState::Parked, false, false),
        ];
        let entries = build_menu(&list, true);
        match &entries[0] {
            MenuEntry::Submenu { enabled, .. } => assert!(*enabled),
            other => panic!("expected submenu, got {other:?}"),
        }
        match &entries[1] {
