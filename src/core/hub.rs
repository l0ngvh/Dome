...
use super::layout::{LayoutOptions, PreferredWorkspace};
use super::matcher::{FloatFullscreenMatcherId, MatcherHit, WindowMatcher, WindowMode};
use super::monitor::{Monitor, ReportedMonitor};
use super::node::{
    Container, ContainerId, DisplayMode, Length, LimitObservation, LimitUpdate, MonitorId,
...
pub(crate) struct HubAccess {
    ...
    pub(super) preferred_layouts: Vec<PreferredWorkspace>,
...
impl Hub {
    pub(crate) fn new(
        ...
        preferred_layouts: Vec<PreferredWorkspace>,
    ) -> Self {
    ...
    pub(crate) fn sync_configuration(&mut self, layout: LayoutOptions) {
    ...
    pub(crate) fn sync_preferred_layout(&mut self, preferred_layouts: Vec<PreferredWorkspace>) {
        self.index_matchers(&preferred_layouts);
