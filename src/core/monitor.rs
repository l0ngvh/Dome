...
use super::node::{Dimension, Length, MonitorId, WorkspaceId};
...
impl Hub {
        ...
        for ws_id in reattaching {
            ...
            if self.access.monitors.get(previous_host).active_workspace == ws_id {
                let own = self
                    ...
                    .all_active()
                    .into_iter()
                    .find(|(_, ws)| ws.monitor == previous_host && ws.is_attached())
                    .map(|(ws_id, _)| ws_id);
    ...
    fn recompute_monitor_names(&mut self) {
        let all: Vec<(MonitorId, String, Dimension)> = self
            .access
            .monitors
            .all_active()
            .into_iter()
            .map(|(id, m)| (id, m.device_name.clone(), m.dimension))
            .collect();
        for (id, device_name, _) in &all {
            let mut colliders: Vec<&(MonitorId, String, Dimension)> =
                all.iter().filter(|(_, dn, _)| dn == device_name).collect();
            let unique = if colliders.len() == 1 {
                device_name.clone()
            } else {
                colliders.sort_by(|(_, _, a), (_, _, b)| {
                    a.x.partial_cmp(&b.x)
                        .expect("monitor coords are finite")
                        .then(a.y.partial_cmp(&b.y).expect("monitor coords are finite"))
                });
                let rank = colliders
                    .iter()
