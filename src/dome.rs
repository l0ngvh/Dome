...
define_class!(
    ...
    unsafe impl NSApplicationDelegate for AppDelegate {
        ...
        fn did_finish_launching(&self, _notification: &NSNotification) {
            ...
            let context_ptr = Box::into_raw(Box::new(WindowContext::new(overlay_view)));
...
impl WindowContext {
    fn new(overlay_view: Retained<OverlayView>) -> Self {
...
fn setup_app_observers(context_ptr: *mut WindowContext) -> Observers {
...
fn update_overlay(context: &WindowContext) {
    ...
    if let Some(root) = context.hub.get_workspace(workspace_id).root() {
        let (rects, labels) = collect_overlays(context, root);
...
fn collect_overlays(context: &WindowContext, root: Child) -> (Vec<OverlayRect>, Vec<OverlayLabel>) {
                ...
                if container.is_tabbed() {
                    ...
                    if !children.is_empty() {
                        ...
                        for (i, c) in children.iter().enumerate() {
                            let label = match c {
                                Child::Window(wid) => {
                                    context.registry.borrow().get(*wid).unwrap().title()
                                }
        ...
        match focused {
            Child::Window(window_id) => {
                ...
                rects.extend(border_rects(
                    ...
                    border_size,
                    false,
                    [color.clone(), bottom, color, right],
                ));
            }
            Child::Container(container_id) => {
                let container = context.hub.get_container(container_id);
