use crate::config::bootstrap::{
    LUARC_JSON, META_LUA, STARTER_LUA, bootstrap_default_path, resolve_config_path,
    write_editor_support,
};
...
#[test]
fn an_explicit_path_writes_nothing() {
    let dir = temp_config_dir("explicit_path");
    let _cleanup = CleanupDir(dir.clone());
    std::fs::create_dir_all(&dir).unwrap();
    let path = dir.join("custom.lua");

    let resolved = resolve_config_path(Some(path.to_str().unwrap().to_string()));
...
fn bootstrap_writes_starter_config_when_missing() {
    ...
    bootstrap_default_path(path.to_str().unwrap());
