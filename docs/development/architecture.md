...
**HICON source cap.** `DrawIconEx` captures from the HICON returned by the resolver's first successful probe, typically a 32x32 handle. Captures above ~48 physical pixels (scale > 2.0) show interpolation blur because `DrawIconEx` has only that 32x32 source to work from. This is a known limitation. The follow-up APIs are `PrivateExtractIconsW` (user32) and `LoadIconWithScaleDown` (commctrl.h), which can select higher-resolution authored variants from the PE resources.
