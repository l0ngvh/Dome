...
- `render.rs` -- DPI scale assignment and picker scale (Windows only)

Platform tests don't use snapshots -- they assert on concrete state values and mock call counts. Prefer asserting on observable behavior (mock call logs, returned dimensions, move counts) over internal struct fields. If a test needs `#[cfg(test)]` helpers that expose internal state just for assertions, that's a sign the test is too coupled to implementation details. Rewrite it to verify what the code *does* (the effects visible through trait boundaries) rather than what it *stores*.
